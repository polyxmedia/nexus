...
export async function POST(request: NextRequest) {
  try {
    ...
    const rows = await db.insert(schema.predictions).values({ claim, timeframe, deadline, confidence, category, signalId: signalId || null, analysisId: analysisId || null, metrics: metrics ? JSON.stringify(metrics) : null }).returning();
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!id || !outcome) return NextResponse.json({ error: "id and outcome are required" }, { status: 400 });
    const existingRows = await db.select().from(schema.predictions).where(eq(schema.predictions.id, id));
    if (!existingRows[0]) return NextResponse.json({ error: "Prediction not found" }, { status: 404 });
    const updatedRows = await db.update(schema.predictions).set({ outcome, outcomeNotes: outcomeNotes || null, score: score !== undefined ? score : null, resolvedAt: new Date().toISOString() }).where(eq(schema.predictions.id, id)).returning();
