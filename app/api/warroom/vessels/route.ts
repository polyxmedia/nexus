...
function generateVessels(): VesselState[] {
  const now = Date.now();

  // On first call, build from seed data
  if (!cachedVessels) {
  ...
  // Subsequent calls: drift positions slightly to simulate movement
  const elapsedHours = (now - cacheTimestamp) / 3_600_000;
  cachedVessels = cachedVessels.map((v) => {
    const courseRad = (v.course * Math.PI) / 180;
    // Approximate: 1 knot = ~0.0003 degrees/minute lat drift at mid latitudes
    const drift = v.speed * elapsedHours * 0.0166; // rough degrees per elapsed time
