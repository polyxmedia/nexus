    ...
    const rows = await db
      ...
      .values({
        ...
        metrics: JSON.stringify({
          ...(p.grounding ? { grounding: p.grounding } : {}),
          calibration: {
            rawConfidence: p.confidence,
            baseRateAdjusted: baseConfidence,
            thresholdDiscount: thresholdResult.discountMultiplier,
            thresholdPercentile: thresholdResult.percentileRank,
            plattCalibrated,
            finalConfidence,
            magnitudeCap: magnitudeConfidenceCap,
          },
        }),
...
You MUST call submit_resolution before finishing. Never end without submitting.`;
  ...
  // Verification pass disabled: was overturning correct confirmations due to
  // reasoning errors (wrong year, bad math, missing tool access). The resolver's
  // 10-round tool loop with explicit prompting is sufficient.
