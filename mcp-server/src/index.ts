...
server.tool(
  ...
  async () => {
    try {
      ...
      const [signals, predictions, confirmed, resolved, users, activeSubs, chatSessions, knowledge, trades] =
...
server.tool(
  "get_prediction",
  "Get a specific prediction by numeric ID or UUID, including signal and analysis data",
  {
  ...
  async ({ id }) => {
    try {
      ...
      const result = await pool.query(
        `SELECT p.*, s.title as signal_title, s.category as signal_category, s.intensity as signal_intensity
         FROM predictions p
         LEFT JOIN signals s ON p.signal_id = s.id
         WHERE ${isUuid ? "p.uuid" : "p.id"} = $1`,
        [isUuid ? id : parseInt(id)]
      );
      if (result.rows.length === 0) return textResult("Prediction not found");
...
server.tool(
  "list_predictions",
  "List predictions with optional filters: pending, resolved, by category, overdue, etc.",
  {
  ...
  async ({ status, category, limit }) => {
    try {
      ...
      if (status === "pending") { where += " AND p.outcome IS NULL"; }
      else if (status === "resolved") { where += " AND p.outcome IS NOT NULL"; }
      else if (status === "overdue") { where += ` AND p.outcome IS NULL AND p.deadline <= $${idx}`; params.push(new Date().toISOString().split("T")[0]); idx++; }

      if (category) { where += ` AND p.category = $${idx}`; params.push(category); idx++; }

      params.push(limit);
      const result = await pool.query(
        `SELECT p.id, p.uuid, p.claim, p.category, p.confidence, p.outcome, p.score, p.deadline, p.direction, p.reference_symbol, p.created_at, p.resolved_at, p.created_by
         FROM predictions p ${where}
         ORDER BY p.created_at DESC LIMIT $${idx}`,
        params
      );
      return textResult(result.rows);
    } catch (err) {
...
server.tool(
  "update_prediction",
  "Update a prediction's fields (outcome, score, notes, deadline, confidence, etc.)",
  {
    id: z.string().describe("Prediction ID (numeric) or UUID"),
  ...
  async ({ id, outcome, outcome_notes, score, deadline, confidence, direction_correct, level_correct }) => {
    try {
      ...
      if (outcome !== undefined) { sets.push(`outcome = $${idx}`); params.push(outcome); idx++; sets.push(`resolved_at = $${idx}`); params.push(new Date().toISOString()); idx++; }
      if (outcome_notes !== undefined) { sets.push(`outcome_notes = $${idx}`); params.push(outcome_notes); idx++; }
      if (score !== undefined) { sets.push(`score = $${idx}`); params.push(score); idx++; }
      if (deadline !== undefined) { sets.push(`deadline = $${idx}`); params.push(deadline); idx++; }
      if (confidence !== undefined) { sets.push(`confidence = $${idx}`); params.push(confidence); idx++; }
      if (direction_correct !== undefined) { sets.push(`direction_correct = $${idx}`); params.push(direction_correct); idx++; }
      if (level_correct !== undefined) { sets.push(`level_correct = $${idx}`); params.push(level_correct); idx++; }

      if (sets.length === 0) return textResult("No fields to update");

      params.push(isUuid ? id : parseInt(id));
      const result = await pool.query(
        `UPDATE predictions SET ${sets.join(", ")} WHERE ${isUuid ? "uuid" : "id"} = $${idx} RETURNING id, uuid, claim, outcome, score, deadline`,
        params
      );
      if (result.rows.length === 0) return textResult("Prediction not found");
...
server.tool(
  "prediction_accuracy",
  "Get prediction accuracy stats: overall, by category, Brier scores, hit/miss rates",
  {},
  async () => {
    try {
...
server.tool(
  "list_signals",
  "List signals with optional filters by category, status, or intensity",
  {
  ...
  async ({ category, status, min_intensity, limit }) => {
    try {
      ...
      if (min_intensity) { where += ` AND intensity >= $${idx}`; params.push(min_intensity); idx++; }

      params.push(limit);
      const result = await pool.query(
        `SELECT id, uuid, title, description, category, intensity, status, layers, market_sectors, created_at
         FROM signals ${where} ORDER BY created_at DESC LIMIT $${idx}`,
        params
      );
      return textResult(result.rows);
    } catch (err) {
...
server.tool(
  "list_blog_posts",
  "List blog posts with status, publication date, and metadata",
  {
  ...
  async ({ status, limit }) => {
    try {
      ...
      if (status !== "all") { where = `WHERE status = $${idx}`; params.push(status); idx++; }

      params.push(limit);
      const result = await pool.query(
        `SELECT id, slug, title, status, category, author, published_at, created_at, tweet_id,
                LENGTH(content) as content_length
         FROM blog_posts ${where} ORDER BY created_at DESC LIMIT $${idx}`,
        params
      );
      return textResult(result.rows);
    } catch (err) {
...
server.tool(
  "get_blog_post",
  "Get a full blog post by ID or slug, including content",
  {
  ...
  async ({ id }) => {
    try {
      ...
      const result = await pool.query(
        `SELECT * FROM blog_posts WHERE ${isNumeric ? "id" : "slug"} = $1`,
        [isNumeric ? parseInt(id) : id]
      );
      if (result.rows.length === 0) return textResult("Blog post not found");
      return textResult(result.rows[0]);
    } catch (err) {
      return errorResult(err);
    }
...
server.tool(
  "update_blog_post",
  "Update a blog post's content, status, title, or metadata",
  {
    id: z.string().describe("Blog post ID (numeric) or slug"),
  ...
  async ({ id, title, content, excerpt, status, category, tags }) => {
    try {
      ...
      if (excerpt !== undefined) { sets.push(`excerpt = $${idx}`); params.push(excerpt); idx++; }
      if (status !== undefined) {
        sets.push(`status = $${idx}`); params.push(status); idx++;
        if (status === "published") { sets.push(`published_at = $${idx}`); params.push(new Date().toISOString()); idx++; }
      }
      if (category !== undefined) { sets.push(`category = $${idx}`); params.push(category); idx++; }
      if (tags !== undefined) { sets.push(`tags = $${idx}`); params.push(tags); idx++; }

      sets.push(`updated_at = $${idx}`); params.push(new Date().toISOString()); idx++;

      if (sets.length <= 1) return textResult("No fields to update");

      params.push(isNumeric ? parseInt(id) : id);
      const result = await pool.query(
        `UPDATE blog_posts SET ${sets.join(", ")} WHERE ${isNumeric ? "id" : "slug"} = $${idx} RETURNING id, slug, title, status`,
        params
      );
      if (result.rows.length === 0) return textResult("Blog post not found");
      return textResult(`Updated: ${JSON.stringify(result.rows[0], null, 2)}`);
...
server.tool(
  "credit_balance",
  "Check credit balance for a user or all users",
  {
  ...
  async ({ user_id }) => {
    try {
      ...
      const result = await pool.query(
        `SELECT cb.*, (cb.credits_granted - cb.credits_used) as remaining FROM credit_balances cb ORDER BY cb.updated_at DESC`
      );
      return textResult(result.rows);
    } catch (err) {
      return errorResult(err);
...
server.tool(
  "search_knowledge",
  "Search the knowledge bank by keyword (text search, not vector)",
  {
  ...
  async ({ query, limit }) => {
    try {
      const result = await pool.query(
        `SELECT id, title, source, category, created_at, LENGTH(content) as content_length
         FROM knowledge
...
server.tool(
  ...
  async ({ status, environment, limit }) => {
    try {
      ...
      if (status && status !== "all") { where += ` AND status = $${idx}`; params.push(status); idx++; }
      if (environment) { where += ` AND environment = $${idx}`; params.push(environment); idx++; }

      params.push(limit);
      const result = await pool.query(
        `SELECT id, ticker, direction, quantity, entry_price, current_price, pnl, status, environment, signal_id, prediction_id, created_at
         FROM trades ${where} ORDER BY created_at DESC LIMIT $${idx}`,
        params
      );
      return textResult(result.rows);
    } catch (err) {
...
server.tool(
  "execute",
  "Execute a write SQL query (INSERT, UPDATE, DELETE) against the Nexus database. Use with caution.",
  {
...
server.tool(
  "list_scenarios",
  "List game theory scenarios and their current states",
  {
    limit: z.number().optional().default(10),
  },
  async ({ limit }) => {
    try {
      const result = await pool.query(
        `SELECT ss.id, ss.scenario_id, ss.state, ss.created_at, ss.updated_at,
